# VİRAL MEKANİZMA ANALİZİ
**Analiz tarihi:** 2026-05-09
**Kaynak:** 12 video, 8.1M+ toplam izlenme
**Kullanım:** youtube-strategy.md ve youtube-state-layer.md Viral Mekanizma bölümlerine ekle

---

## KANITLANMIŞ HOOK TİPLERİ

### Hook 1 — "Keşke Biri Söyleseydi" Formülü
**Yapı:** [Pişmanlık bildiren açılış] + [Kimin için geçerli] + [Ne kazanacaklar]
**Örnek:** "Keşke biri bana bu yedi gerçeği daha öncesinde söyleseydi."
**Kaynak:** V4 (300K/6 gün), V5 (250K)
**Neden çalışır:** İzleyiciyi öğrenmenin önüne koyuyor. "Bu benim için" hissi anında kuruluyor.
**Axonode uyarlaması:** "Veri bilimine geçerken keşke biri bana söyleseydi — [X gerçek]."

### Hook 2 — Karşı-sezgisel İddia + Spesifik Sayı
**Yapı:** [Beklenmedik/ters iddia] + [Somut sayı veya veri] + [Kim için]
**Örnek:** "Çoğu insan yapay zekanın düşünme yetilerini yok etmesine izin veriyor. En üstteki %1'lik kesim ise tam tersini yapıyor."
**Kaynak:** V8 (1.7M/3 ay), V5 (250K/5 ay)
**Neden çalışır:** İki zıt grup yaratıyor — izleyici hangi grupta olduğunu merak ediyor.
**Axonode uyarlaması:** "Python öğrenmek artık yeterli değil — 2026'da veri bilimi aslında neyi istiyor?"

### Hook 3 — Kişisel Dürüstlük / İtiraf Açılışı
**Yapı:** [Sürpriz kişisel itiraf] + [İzleyiciyle ortak nokta] + [Ama işte çözüm]
**Örnek:** "Dürüst olmak gerekirse biraz tembelimdir. Aynı zamanda çok da sabırsızım."
**Kaynak:** V1 (197K/5 ay), V10, V11 (760K)
**Neden çalışır:** Uzman kimliği kırılıyor, insan kimliği öne çıkıyor. Güven anında kuruluyor.
**Axonode uyarlaması:** "Veri bilimine geçmeye karar verdiğimde yanlış yaptığım ilk şey şuydu..."

### Hook 4 — Somut Otorite + Zaman Çerçevesi
**Yapı:** [Kimliğini ve süreyi belirt] + [Ne gördün/öğrendin] + [İzleyiciye ne sunacaksın]
**Örnek:** "10 yılı aşkın deneyime sahip bir veri analisti olarak, sıfırdan başlasaydım tamamen farklı bir yol izlerdim."
**Kaynak:** V0 (1M), V8 (1.7M)
**Neden çalışır:** Otorite ve kısayol vaadi birlikte geliyor. "Bu kişi benim yerime hata yaptı" hissi.
**Axonode uyarlaması:** "Sağlıktan veri bilimine geçişin [X]. ayında şunu fark ettim..."

### Hook 5 — Kontrarian Soru — İzleyicinin Kafasındaki Soruyu Çalmak
**Yapı:** [Herkesin sorduğu soruyu yüksek sesle sor] + [Ama bu sefer farklı cevaplayacağım]
**Örnek:** "AI, AI, AI... Peki, bu AI ajanları tam olarak nasıl çalışır?" (V7 — 4M izlenme)
**Kaynak:** V7 (4M), V6 (62K)
**Neden çalışır:** İzleyici "evet, ben de bunu merak ediyordum" hisseder. Arama niyetiyle hook uyuşuyor.
**Axonode uyarlaması:** "Yapay zeka veri bilimcileri işsiz bırakacak mı? Bu soruya dürüst bir cevap verelim."

### Hook 6 — Vaat + Acil Hedef Çerçevesi
**Yapı:** [Herkes yapabilir iddiası] + [Ne kadar sürede] + [Nasıl]
**Örnek:** "Herkes — evet, siz de — sadece altı aylık bir kendi kendine çalışma süreciyle veri analisti olabilir."
**Kaynak:** V0 (1M), V2 (175K)
**Neden çalışır:** Kapsamlı kapsayıcılık + somut süre. İzleyicinin kendini dışarıda bırakmasını engelliyor.
**Axonode uyarlaması:** "AI agent sistemlerini [X] adımda anlayabilirsiniz — teknik geçmiş gerekmez."

---

## RETENTION MEKANİZMALARI

### Mekanizma 1 — Sayılı Liste + Geçiş Kartları
- Her bölüm başında rakam söyle: "Birinci madde...", "İkinci madde..."
- Kaç madde olduğunu önceden söyle — izleyici nerede olduğunu biliyor
- Kaynak: V4 (300K/6 gün) — "7 gerçek" formatı
- Axonode için: Her Trend Analizi videosu için "X trend, X çıkarım" yapısı

### Mekanizma 2 — Gerçek Hayat Analojisi
- Teknik kavramı günlük bir şeyle eşleştir, hiç bırakma
- V4: kodlama = yemek pişirmek, tesisat. V8: yapay zeka = spor salonu. V7: LLM = basit girdi-çıktı makinesi
- Axonode için: veri bilimi = dil öğrenmek, AI agent = araç kutusu, data pipeline = mutfak hazırlığı

### Mekanizma 3 — Tehdit → Çözüm Yolu
- Videoyu rakamlarla aç (tehdit büyük göster) sonra "ama bunun çözümü var" ile devam et
- V5: "100.000 teknoloji çalışanı işten çıkarıldı" → "kariyerinizi AI-geçirmez kılmak için 3 adım"
- Axonode için: Trend Analizi videolarında bu yapıyı kullan

### Mekanizma 4 — Kademeli Sınıflandırma / Seviye Yapısı
- "Seviye 1... Seviye 2... Seviye 3..." formatı izleyiciyi ilerletiyor
- V7 (4M izlenme): LLM → İş Akışı → Ajan — her seviye bir öncekinin üzerine kuruluyor
- Axonode için: Tutorial videolarında "temel → orta → ileri" kademe yapısı kullan

---

## CTA STRATEJİLERİ

### En Güçlü — Yorum CTA + İçerik Çağrısı
**Formül:** "Bir sonraki videoda hangi konuyu anlatmamı istersiniz? Yorumda yazın."
**Neden:** İzleyici içerik sürecine dahil oluyor. Yorum sayısı artar, algoritma sinyali güçlenir.
**Kaynak:** V7 (4M) — "hangi AI ajanı hakkında eğitim çekmemi istersiniz?"

### Orta Güçlü — Sonraki Video Bağlantısı
**Formül:** "Bir sonraki videoda [spesifik konu] anlatacağım — seri devam ediyor."
**Neden:** Watch time zinciri. Her video bir sonrakine kapı açıyor.
**Kaynak:** V4, V11

### Standart — Beğeni Gerekçesi
**Formül:** "Bu videoya beğeni basarsanız daha fazla insana ulaşabiliyoruz."
**Neden:** Beğeninin amacını açıklamak dönüşüm oranını artırıyor.
**Kaynak:** V4

---

## KAÇINILACAKLAR

1. **Görsel bağımlı anlatı** — "Şurada gördüğünüz gibi..." / "Ekranda yazdığı gibi..." Podcast uyumluluğu düşüyor, altyazıda anlam kaybolabiliyor. Konuşma metnini görsel olmadan da anlaşılır yaz.

2. **Belirsiz açılış** — "Bugün size X'i anlatacağım." Bunu direk iddia veya veriyle değiştir. İlk cümle izleyicinin devam kararını veriyor.

3. **Uzun bağlam açıklaması** — V3 (en düşük performans) bu hatayı yapıyor. Kanalı, kendini, ayın nasıl geçtiğini anlatmak için çok zaman harcıyor. Konuya 30 saniye içinde gir.

4. **Sıradan açılış** — "Selamlar, bu videoda..." hiç kullanma. İlk cümle konuşmanın en güçlü noktası olmalı.

---

## EN İYİ PERFORMANS GÖSTEREN YAPI (ÖNERİ)

```
[HOOK — 0-15 saniye]
Karşı-sezgisel iddia veya çarpıcı veri.
Tek cümle. Düşündürücü.

[NEDEN İZLEMELİ — 15-30 saniye]
"Bu videoda [X şey] öğreneceksin" — spesifik, somut.
Sayı ver: "3 araştırma", "5 trend", "1 çerçeve"

[VİDEO VADİ — 30 saniye]
"[X dakika içinde] bu konuyu bitiriyoruz. Başlayalım."

[GEÇİŞ KART — her bölüm başında]
"Birinci madde / Birinci adım / Birinci trend:" formatı

[RETENTION HOOK — her 2-3 dakikada]
"Bir sonraki madde bu arada en önemlisi..." — merak bırak

[ÖZET — son 90 saniye]
Her madde tek cümleyle. En önemli çıkarım. CTA.

[CTA — son 30 saniye]
Yorum sorusu + sonraki video linki + abone neden
```

---

**Bu dosyayı şu bölümlere kopyala:**
- `youtube-strategy.md` → VIRAL MEKANİZMA LİBRARYSİ bölümü
- `youtube-state-layer.md` → VİRAL MEKANİZMA NOTLARI bölümü
- `youtube-production-template.md` → BÖLÜM 8: HOOK ANALİZ NOTLARI bölümü

## HOOK KARAR MATRİSİ — TEK REFERANS

Video tipine göre zorunlu hook sıralaması:

| Video Tipi     | 1. Tercih                  | 2. Tercih              | Yasak               |
|----------------|----------------------------|------------------------|---------------------|
| Tutorial       | Vadi Hook (#6)             | Kademeli Seviye (#4)   | Uzun öz-tanıtım     |
| Trend Analizi  | Karşı-sezgisel Sayı (#2)   | Kontrarian Soru (#5)   | Belirsiz açılış     |
| Kariyer / POV  | İtiraf (#3)                | Keşke Söyleseydi (#1)  | Otorite iddiası     |
| Girişim / Para | Somut Sayı (#2 varyant)    | Vaat (#6 varyant)      | Duygusal soyutlama  |

Güncelleme kuralı:
- Bir hook tipi 2+ videoda %40+ retention getirdiyse → o tip için "Kanıtlanmış" etiketi ekle
- Kanıtlanmış hook varsa ajan 1. tercih yerine kanıtlanmışı seçer
- Kanıtlanmış hook yoksa tablodaki 1. tercih geçerli