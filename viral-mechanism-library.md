# VİRAL MEKANİZMA KÜTÜPHANESİ
**Owner:** Sema | AxonodeAI
**Son Güncelleme:** 2026-05-11
**Toplam Pattern:** 6

Bu dosya /youtube komutu çalıştıkça büyür.
Her pattern birden fazla videoda doğrulandıktan sonra "Kanıtlanmış" olarak işaretlenir.

---

## KANITLANMIŞ SAYILMA KRİTERİ

Bir pattern "Kanıtlanmış" olabilmek için:
- Aynı hook tipi veya başlık formülü **2+ farklı videoda** uygulanmış olmalı
- Bu videoların her ikisinde şu eşiklerden en az biri sağlanmalı:
  - Retention > %40, VEYA
  - CTR > %3, VEYA
  - Beğeni oranı > %5

Tek videoda iyi sonuç = "Test ediliyor" etiketi kalır.
İki videoda eşik sağlandı = "Kanıtlanmış" etiketi verilir.

---

## KANITLANMIŞ PATTERNLER
[Henüz yok — video sayısı artınca burası dolacak]

---

## TEST EDİLİYOR
[Tüm mevcut patternler — tek video verisi var]

---

## PATTERN — SAYI/PARA MOTİVASYONU BAŞLIKTA
**ID:** VPT-001
**Kategori:** Başlık
**Kaynak video:** VPT-001 — Yeni Yapay Zeka Meslekleri: 120K€, Kod Yok, Diploma Yok (2026 Patlayacak!)
**İzlenme:** 50.000+ (tahmini)
**Doğrulama:** 1 video

**Gözlem:**
Başlıkta somut para rakamı veya sayısal veri (120K€, %57, 3 Yol) eklemek CTR'ı artırıyor. "Kod Yok, Diploma Yok" gibi erişim bariyerini kaldıran ifadeler merak + umut kombinasyonu oluşturuyor.

**Neden çalışıyor:**
İzleyici somut rakamı görünce videonun kendisine ne kazandıracağını anında hesaplıyor. Belirsiz "iyi gelecek" yerine "120K€" beyin için işlenebilir ve tıklamayı tetikliyor.

**AxonodeAI uygulaması:**
Girişim veya kariyer videolarında başlığa somut rakam ekle. Örnek: "AI Agent Freelance: Ayda 3 Proje ile Nasıl Para Kazanılır" veya "Veri Bilimciler %40 Daha Fazla Kazanıyor — 2026 Araştırması"

**Kullanıldığı videolarım:**
- Henüz kullanılmadı

**Ekleme tarihi:** 2026-05-09
**Son güncelleme:** 2026-05-09

---

## PATTERN — YIL + ETKİ İDDİASI KOMBİNASYONU
**ID:** VPT-002
**Kategori:** Başlık
**Kaynak video:** VPT-002 — 2025'te Hayatımızı Değiştirecek AI Agents Nedir?
**İzlenme:** 50.000+ (tahmini)
**Doğrulama:** 1 video

**Gözlem:**
Başlığın başına yıl + büyük etki iddiası ("Hayatımızı Değiştirecek", "Her Şeyi Değiştiriyor") koymak, konunun hem güncel hem de kritik olduğunu sinyal ediyor. İzleyici "neden şimdi?" sorusunu sormadan cevap alıyor.

**Neden çalışıyor:**
YouTube öneri algoritması "güncel mi?" sorusunu kullanıcıdan önce soruyor. Başlıkta yıl + büyüklük iddiası hem algoritmaya hem izleyiciye aynı anda "bu yeni ve önemli" mesajı veriyor.

**AxonodeAI uygulaması:**
Her Trend Analizi başlığında yıl zorunlu. Etki iddiası konuya göre seç: "Değişiyor", "Yetmiyor", "Patlayacak", "Ne Anlama Geliyor?" — bunlar VID-001'de de kullanıldı, doğrulama bekliyor.

**Kullanıldığı videolarım:**
- VID-001 — "Python Öğrenmek Yetmiyor — 2026'da Veri Bilimi Gerçekten Ne İstiyor?" (doğrulama bekleniyor)

**Ekleme tarihi:** 2026-05-09
**Son güncelleme:** 2026-05-09

---

## PATTERN — KONTRAST ÇİFTİ TEKNİĞİ
**ID:** VPT-003
**Kategori:** Başlık + Hook
**Kaynak video:** VPT-003 — IT ve AI Sektöründe Yükselen ve Yok Olan Meslekler
**İzlenme:** 50.000+ (tahmini)
**Doğrulama:** 1 video

**Gözlem:**
"Yükselen / Yok Olan", "Kazananlar / Kaybedenler", "Yapanlar / Yapmayanlar" gibi zıt çiftler hem merak hem kayıp korkusu (FOMO) üretiyor. İzleyici kendi pozisyonunu bulmak için videoyu izliyor.

**Neden çalışıyor:**
Beyin zıtlıkları hızlı işler. Kontrast çifti gördüğünde "ben hangisindeyim?" sorusu otomatik tetikleniyor — bu soru cevap aramak için izlemeye dönüşüyor.

**AxonodeAI uygulaması:**
"Veri Bilimciler Kazanıyor, Kimler Kaybediyor? — 2026" veya "AI ile Büyüyenler ve Geride Kalanlar" tarzı Trend Analizi başlıklarında kullan. Thumbnail'da da görsel kontrast yap (yeşil/kırmızı).

**Kullanıldığı videolarım:**
- Henüz kullanılmadı

**Ekleme tarihi:** 2026-05-09
**Son güncelleme:** 2026-05-09

---

## PATTERN — VADİ HOOK (TUTORIAL TİPİ)
**ID:** VPT-002b
**Kategori:** Hook
**Kaynak video:** VPT-002 + İngilizce LangChain/CrewAI tutorial videoları
**İzlenme:** 100.000+ (İngilizce kanallar)
**Doğrulama:** Birden fazla video (İngilizce)

**Gözlem:**
Tutorial videolarında ilk 30 saniyede "Bu videoda şunu yapacaksın: [somut çıktı]" vadi hook'u yüksek retention üretiyor. İzleyici ne öğreneceğini bilince kaygı azalıyor, izlemeye devam ediyor.

**Neden çalışıyor:**
Belirsizlik bırakmıyor. İzleyici "bu bana uygun mu?" sorusunu sormadan önce cevap alıyor. Somut çıktı vaat edildiğinde izleme süresi artıyor çünkü izleyici o çıktıya ulaşmak için izliyor.

**AxonodeAI uygulaması:**
VID-002 hook: "Bu videonun sonunda LangChain ile çalışan bir veri analizi ajanı kurmuş olacaksın. Kod editörünü aç, birlikte yapıyoruz."

**Kullanıldığı videolarım:**
- VID-002 (planlı — SEO paketi 2026-05-10 hazırlandı)

**Ekleme tarihi:** 2026-05-09
**Son güncelleme:** 2026-05-10

---

## PATTERN — CANLI İSTATİSTİK + SEKTÖR BASINCI
**ID:** VPT-005
**Kategori:** Hook + Başlık
**Kaynak:** Web araştırması — Alphamatch AI, DevTo, Analytics Vidhya (2026-05-10)
**Doğrulama:** 1 kaynak seti (doğrulanıyor)

**Gözlem:**
"Mart 2026 itibarıyla şirketlerin %57'si AI agent sistemlerini üretime aldı" gibi gerçek zamanlı sektör istatistikleri başlık veya hook'ta kullanıldığında merak + FOMO kombinasyonu üretiyor. İzleyici "bu beni de etkiliyor mu?" sorusunu sormadan videonun içine çekiliyor.

**Neden çalışıyor:**
Belirsiz "yapay zeka hızla büyüyor" yerine "%57" gibi somut veri beyne işlenebilir. Kanalın araştırma credibility'sini anında kurar. İzleyici kitlesi profesyoneller olduğunda "bu rakamı iş hayatımda kullanabilirim" hissi var.

**AxonodeAI uygulaması:**
Trend Analizi videolarında hook ve başlıkta güncel sektör istatistiği zorunlu ekle. Kaynak belirt — güvenilirlik artırıyor. Örnek: "%57 Şirket AI Ajanı Benimsedi — Veri Bilimciler Ne Yapıyor?" (VID-004 başlığı)

**Kullanıldığı videolarım:**
- VID-004 (planlı — 2026-05-28)

**Ekleme tarihi:** 2026-05-10
**Son güncelleme:** 2026-05-10

---

## PATTERN — VADİ HOOK + RETENTİON BAĞLANTISI
**ID:** VPT-006
**Kategori:** Hook + Retention
**Kaynak:** VID-001 retention verisi analizi (2026-05-10) + İngilizce tutorial viral videoları
**İzlenme:** VID-001 baz — %20.3 retention (vadi hook yok); İngilizce tutoriallar %45-55 retention (vadi hook var)
**Doğrulama:** Çapraz analiz (doğrulanıyor)

**Gözlem:**
VID-001'de ilk 30 saniyede somut çıktı vaadi yok — retention %20.3'te kalmış. İngilizce viral tutorial videolarında vadi hook zorunlu ve retention %45-55 bandında. Doğrudan nedensellik var: izleyici somut çıktıyı bilince izlemeyi sürdürüyor, bilmeyince 2. dakikada düşüyor.

**Neden çalışıyor:**
Belirsizlik yaratan içerik izleyiciyi erken bırakmaya zorluyor. "Bu bana ne kazandıracak?" sorusu her saniye sorulur — cevap yoksa çıkış. Vadi hook bu soruyu başta kapatıyor, izleyicinin tamamlama güdüsü devreye giriyor.

**AxonodeAI uygulaması:**
Her Tutorial ve Trend Analizi videosunda ilk 30 saniyede mutlaka Vadi Hook ekle:
- Tutorial: "Bu videonun sonunda [somut çıktı] olacak — [araç/kod] hazırla."
- Trend: "Bu videoyu izleyince [spesifik şeyi] yapabileceksin — X dakika yeter."
Yeterince spesifik olmayan vadi çalışmıyor. "Çok şey öğreneceksin" yerine "LangChain ajanı kuracaksın."

**Kullanıldığı videolarım:**
- VID-002 (planlandı — 2026-05-14, bu pattern uygulanacak)

**Ekleme tarihi:** 2026-05-10
**Son güncelleme:** 2026-05-10

---